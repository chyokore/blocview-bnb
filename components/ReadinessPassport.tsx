...
export function ReadinessPassport({ agent, evidence }: { agent: LiveAgent; evidence: EvidenceRecord }) {
  ...
  const pending = agent.source === "range-pilot-watch";
  return <section className="passport" aria-labelledby="passport-title">
    <header className="passport-header"><div><span className="eyebrow">Research record</span><h2 id="passport-title">Agent Readiness Passport</h2><p>See what this record supports and what it does not.</p></div><div className="coverage-count"><strong>{evidence.coverage.available} of {evidence.coverage.total}</strong><span>evidence areas covered</span><small>Coverage, not a score or rating</small></div></header>
    ...
    <div className="passport-grid">
      <PassportSection number="01" title="Identity" state="Available"><dl className="passport-facts"><div><dt>Agent name</dt><dd>{agent.name ?? "Not available"}</dd></div><div><dt>ERC-8004 token ID</dt><dd>{agent.tokenId}</dd></div><div><dt>Network</dt><dd>{agent.network} · Chain ID {agent.chainId}</dd></div><div><dt>Registration timestamp</dt><dd>{formatDate(agent.registeredAt)}</dd></div><div><dt>Retrieved at</dt><dd>{formatDate(evidence.retrieval.retrievedAt)}</dd></div><div><dt>Registry state</dt><dd className={pending ? "" : "verified-text"}>{pending ? "Declared in public registration · 8004scan indexing pending" : <><CheckIcon /> Registry record returned by 8004scan</>}</dd></div></dl></PassportSection>

      <PassportSection number="02" title="Declared capabilities" state={agent.capabilities.length ? "Declared only" : "Not available"}><p className="passport-helper">The agent declared this capability. BLOCview has not independently verified it.</p>{agent.capabilities.length ? <div className="capability-list">{agent.capabilities.map((capability) => <span key={capability}>{capability}</span>)}</div> : <p className="not-available-copy">No declared capabilities were returned.</p>}</PassportSection>

      <PassportSection number="03" title="Reputation" state={hasReputation ? "Available" : "Not available"}><p className="passport-helper">{pending ? "No reputation evidence is available while 8004scan indexing is pending." : "Reputation is shown as returned by 8004scan and is not independently audited by BLOCview."}</p><dl className="reputation-facts"><div><dt>Reputation score</dt><dd>{returned(agent.reputation?.score)}</dd></div><div><dt>Stars</dt><dd>{returned(agent.reputation?.stars)}</dd></div><div><dt>Feedback count</dt><dd>{returned(agent.reputation?.feedbackCount)}</dd></div></dl></PassportSection>
