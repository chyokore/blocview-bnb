...
export function LiveAgentList({ agents }: { agents: LiveAgent[] }) {
  ...
  return <>
    ...
    {filtered.length ? <div className="agent-grid live-grid">{filtered.map((agent) => <article className="agent-card live-card" key={`${agent.chainId}:${agent.tokenId}`}>
      <div className="card-topline"><div className="agent-identity"><span className="agent-avatar live-avatar">{(agent.name?.slice(0, 2) || "#").toUpperCase()}</span><div><h3>{text(agent.name)}</h3><span>ERC-8004 #{agent.tokenId}</span></div></div><span className="source-badge verified">Verified BNB Chain data</span></div>
