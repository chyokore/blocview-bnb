  ...
  return <>
    ...
    {open && <div className="modal-backdrop" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && close()}>
      <section className="activation-modal" role="dialog" aria-modal="true" aria-labelledby="activation-title">
        ...
        {step === 4 && <div className="modal-content confirm-state activation-success" role="status"><span className="confirm-icon"><CheckIcon /></span><h3>Demo activation ready</h3><p className="success-message">The demo is ready. No wallet is connected and no funds have moved.</p><div className="activation-summary"><span>Agent <strong>{agent.name}</strong></span><span>Preference <strong>{preference}</strong></span></div></div>}
