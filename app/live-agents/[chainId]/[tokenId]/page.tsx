...
import { classifyMarketplaceInterface } from "@/lib/live-marketplace";
import { getRangePilotLiveAgent, isRangePilotIndexingPending, isRangePilotLiveAgent, mergeRangePilotIndexedAgent } from "@/lib/range-pilot-watch-agents";
...
export default async function LiveAgentPage({ params }: { params: Promise<{ chainId: string; tokenId: string }> }) {
  ...
  const registration = getRangePilotLiveAgent(chainId, tokenId);
  const indexed = await getLiveAgent(chainId, tokenId);
  const agent = registration ? mergeRangePilotIndexedAgent(registration, indexed) : indexed;
  if (!agent) notFound();
  const evidence = deriveEvidenceRecord(agent);
  const firstParty = isRangePilotLiveAgent(agent);
  const pending = firstParty && isRangePilotIndexingPending(agent);
  const externalInterface = firstParty ? null : classifyMarketplaceInterface(agent);
  return <main><Header /><div className="detail-shell live-detail">
    ...
    <section className="detail-hero"><div className="detail-main"><div className="source-badge verified">{pending ? "Public registration · indexing pending" : "8004scan registry record"}</div><div className="agent-title"><span className="agent-avatar large live-avatar">{(agent.name?.slice(0, 2) || "#").toUpperCase()}</span><div><span className="eyebrow">{firstParty ? agent.category : externalInterface ? `Interface · ${externalInterface}` : "Interface not returned"}</span><h1>{show(agent.name)}</h1></div></div><p>{show(agent.description)}</p></div><aside className="source-panel"><span>Data source</span><strong>{pending ? "Public ERC-8004 registration JSON" : "8004scan Public API"}</strong><p>{pending ? "This identity is linked to the stated BSC registry. 8004scan has not indexed, rated, validated, or checked the agent's operation yet." : "BLOCview fetched this record from its server. Demo strategy, performance, risk, and activation data are not added to this profile."}</p></aside></section>
    {firstParty && <><section className="readiness-passport"><div className="section-heading"><span className="eyebrow">Evidence checkpoint</span><h2>Check the sources before the test</h2><p>Review this agent&apos;s registration, documentation, and health response before running an assessment. The POST destination is fixed. You cannot supply another URL, chain, contract, or RPC endpoint.</p></div><div className="passport-grid"><a className="secondary-button" href={agent.registrationUrl} target="_blank" rel="noreferrer">View registration</a><a className="secondary-button" href={agent.documentationUrl} target="_blank" rel="noreferrer">View documentation</a><a className="secondary-button" href={agent.healthUrl} target="_blank" rel="noreferrer">Check health</a></div></section><ReadOnlyAssessment agent={agent} /><HireAgent agent={agent} /></>}
