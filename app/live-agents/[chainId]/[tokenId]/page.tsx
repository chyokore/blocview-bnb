...
export default async function LiveAgentPage({ params }: { params: Promise<{ chainId: string; tokenId: string }> }) {
  ...
  return <main><Header /><div className="detail-shell live-detail">
    ...
    {firstParty && <><section className="readiness-passport"><div className="section-heading"><span className="eyebrow">Evidence checkpoint</span><h2>Check the sources before the test</h2><p>Review this agent&apos;s registration, documentation, and health response before running an assessment. The POST destination is fixed. You cannot supply another URL, chain, contract, or RPC endpoint.</p></div><div className="passport-grid"><a className="secondary-button" href={agent.registrationUrl} target="_blank" rel="noreferrer">View registration</a><a className="secondary-button" href={agent.documentationUrl} target="_blank" rel="noreferrer">View documentation</a><a className="secondary-button" href={agent.healthUrl} target="_blank" rel="noreferrer">Check health</a></div></section><ReadOnlyAssessment agent={agent} /></>}
