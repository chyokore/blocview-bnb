...
export default async function LiveAgentPage({ params }: { params: Promise<{ chainId: string; tokenId: string }> }) {
  ...
  return <main><Header /><div className="detail-shell live-detail">
    ...
    {pending && <><section className="readiness-passport"><div className="section-heading"><span className="eyebrow">Reviewed external handoff</span><h2>Read-only service</h2><p>Review the agent-specific documentation and health response before requesting one assessment. The external POST destination is fixed and no arbitrary URL, chain, contract, or RPC input is accepted.</p></div><div className="passport-grid"><a className="secondary-button" href={agent.documentationUrl} target="_blank" rel="noreferrer">Documentation</a><a className="secondary-button" href={agent.healthUrl} target="_blank" rel="noreferrer">Agent service health</a><a className="secondary-button" href={agent.registrationUrl} target="_blank" rel="noreferrer">Registration evidence</a></div></section><ReadOnlyAssessment agent={agent} /></>}
